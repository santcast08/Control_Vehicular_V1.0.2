 ...
 */
    ...
    $SC_cod_proj = "Control_Vehicular";
    $SC_apl_proc = "grid_movilizacion_impresion";
    $SC_ver_93   = false;
/* sc_apl_default */
            ...
            <div class="input-group input-group-vertical">
              <?php
                if ($bookmarks != "XX" && $conf_socor == "N")
                 {
                    $sDisplay = ($opc == 'pdf_det' || $tem_gb_pdf != "s") ? ' style="display: none"' : '';
              ...
              if (in_array("grid", $all_modules) !== false && $SC_ver_93)
              {
                ...
                $sDisplay = (!$ckeck_grid) ? ' style="display: none"' : '';
                ?>
                ...
                $sDisplay = (!$ckeck_grid) ? ' style="display: none"' : '';
                ?>
                <label id="id_grid_label" <?php echo $sDisplay; ?>><input type="checkbox" id="id_all_label" name="all_label" value="<?php echo $all_label; ?>" <?php echo $check; ?>><?php echo $tradutor[$language]['page_label']; ?></label>
                <?php
                $check = ($label_group == "S") ? " checked" : "";
                $sDisplay = ($tem_gb_pdf != "s" || $origem == "res" || !$ckeck_grid) ? ' style="display: none"' : '';
                ?>
...
// Charts
        ...
        $isMultiSeriesChart = $is_chart_app && (!isset($_SESSION['sc_session'][$script_case_init]['grid_movilizacion_impresion']['summarizing_drill_down']) || !$_SESSION['sc_session'][$script_case_init]['grid_movilizacion_impresion']['summarizing_drill_down']);
        $isAnalitic         = isset($_SESSION['sc_session'][$script_case_init]['grid_movilizacion_impresion']['graf_opc_atual']) && 2 == $_SESSION['sc_session'][$script_case_init]['grid_movilizacion_impresion']['graf_opc_atual'];

if ($ver_93 == "n")
{
          $sDisplay = !isset($_SESSION['sc_session']['grid_movilizacion_impresion']['show_skip_charts_option']) ||
                     !$_SESSION['sc_session']['grid_movilizacion_impresion']['show_skip_charts_option'] ||
                     (isset($_SESSION['sc_session'][$script_case_init]['grid_movilizacion_impresion']['SC_Ind_Groupby']) && '_NM_SC_' == $_SESSION['sc_session'][$script_case_init]['grid_movilizacion_impresion']['SC_Ind_Groupby']) ||
                     $isMultiSeriesChart ||
                     $isAnalitic ||
                     ($opc == 'pdf_det')
                     ? ' style="display: none"' : '';
        ...
        {
         $sDisplay =
//                    !isset($_SESSION['sc_session']['grid_movilizacion_impresion']['show_skip_charts_option']) ||
//                    !$_SESSION['sc_session']['grid_movilizacion_impresion']['show_skip_charts_option'] ||
                    $isMultiSeriesChart ||
                    $isAnalitic ||
                     ($opc == 'pdf_det')
...
?>
     ...
     parent.nm_gp_move('<?php echo NM_encode_input($opc); ?>', '<?php echo NM_encode_input($target); ?>', cor, parms_pdf, grafico, create, '<?php echo NM_encode_input($export_ajax); ?>', chart_level, page_break, res_cons, use_pass, s_all_cab, s_all_label, s_label_group);return false;
  }
