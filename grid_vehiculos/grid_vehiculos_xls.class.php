...
class grid_vehiculos_xls
{
   ...
   function grava_arquivo()
   {
      ...
      { 
          ...
          if ($Cada_col == "vehiculo_km_inicial" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $this->count_span++;
              $current_cell_ref = $this->calc_cell($this->Xls_col);
              if (!NM_is_utf8($SC_Label))
   ...
   //----- vehiculo_km_inicial
   function NM_sub_cons_vehiculo_km_inicial()
   {
         if (!NM_is_utf8($this->vehiculo_km_inicial))
         {
             $this->vehiculo_km_inicial = sc_convert_encoding($this->vehiculo_km_inicial, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->vehiculo_km_inicial;
...
<HEAD>
 <TITLE>Reporte de Vehículos :: Excel</TITLE>
